...
A powerful cross-platform Flutter mobile application for intelligent meeting scheduling with AI-powered natural language processing using Google Gemini API. Features recurring meetings, templates, analytics dashboard, meeting link generator, and smart notifications. Perfect for students, professionals, and teams managing their schedules efficiently.
...
- **Automatic Scheduling**: Create daily, weekly, or monthly recurring meetings
- **Custom Intervals**: Set custom repeat patterns (every 2 days, every 3 weeks, etc.)
- **End Date Control**: Specify when recurring series should stop
- **Batch Creation**: All meeting instances created automatically
- **Easy Management**: Each occurrence can be edited or deleted independently

### 📋 Meeting Templates (NEW!)
- **Pre-made Templates**: 4 default templates included (Daily Standup, 1-on-1, Team Meeting, Sprint Planning)
- **Custom Templates**: Create your own reusable meeting templates
- **Quick Apply**: One-tap to pre-fill meeting details
- **Time Saver**: Set up common meetings in seconds
- **Template Management**: Full CRUD operations for templates

### 📝 Meeting Notes (NEW!)
- **Flexible Notes**: Add agenda, discussion points, or attachment references
- **Optional Field**: Use notes when needed, skip when not
- **Persistent Storage**: Notes saved with each meeting
- **Easy Viewing**: Notes display in meeting details view
...
lib/
├── models/                 # Data models
│   ├── meeting.dart           # Meeting model with recurring & notes fields
│   ├── meeting_template.dart  # Template model (NEW)
│   └── chat_message.dart      # Chat message model
├── services/              # Business logic services
│   ├── database_service.dart      # SQLite database (version 3)
│   ├── recurrence_service.dart    # Recurring meeting logic (NEW)
...
│   ├── templates_screen.dart           # Template management (NEW)
│   ├── meeting_details_screen.dart     # Details with link & notes display
│   ├── add_edit_meeting_screen.dart    # Create/edit with templates & recurring
│   ├── analytics_dashboard_screen.dart # Analytics with charts
│   └── settings_screen.dart            # Settings with template access
...
2. **Optional**: Tap 📄 template icon to apply a template
3. Fill in meeting details:
   ...
   - Notes (optional, for agenda or action items)
4. Configure reminders (At start, 15 min, 1 hour, 1 day before)
5. **Optional**: Enable recurring meeting:
   - Toggle "Make this recurring"
   - Select frequency (Daily/Weekly/Monthly)
   - Set interval (every N days/weeks/months)
   - Choose end date
6. Tap "Create Meeting"
7. (Optional) Share invitation with participants
...
1. Open Add/Edit Meeting screen
2. Scroll to "Recurring Meeting" section
3. Toggle "Make this recurring" switch ON
4. Select frequency: Daily, Weekly, or Monthly
5. Set interval (e.g., every 2 days, every 3 weeks)
6. Choose end date for the series
7. Save - all instances are created automatically!

**Examples:**
- Daily standup for 2 weeks: Daily, interval 1, end date 14 days from now
- Weekly team meeting for 3 months: Weekly, interval 1, end date 12 weeks from now
- Bi-weekly 1-on-1: Weekly, interval 2, end date as needed

### 📋 Using Templates

**Apply Templates:**
1. When creating a new meeting, tap the 📄 template icon (top-right)
2. Browse and select a template
3. Meeting details auto-fill from template
4. Adjust date/time and save

**Manage Templates:**
1. Go to Settings → "Manage Templates"
2. Create, edit, or delete templates
3. Set default duration, category, reminders, etc.

**Default Templates Included:**
- Daily Standup (15 min, Work category)
- 1-on-1 Meeting (30 min, Personal category)
- Team Meeting (60 min, Work category)
- Sprint Planning (120 min, Work category)

### 📝 Adding Notes to Meetings

**Add Notes:**
1. In Add/Edit Meeting screen, find the "Notes" field
2. Enter agenda, action items, or any meeting-related info
3. Notes are saved with the meeting

**View Notes:**
- Open meeting details to see notes
- Notes display with a 📝 icon
- Optional field - only shows when present
...
CREATE TABLE meetings (
  ...
  notes TEXT,                          -- NEW in v3: Meeting notes
  isRecurring INTEGER DEFAULT 0,       -- NEW in v3: Is recurring meeting
  recurrenceRule TEXT,                 -- NEW in v3: daily/weekly/monthly
  recurrenceInterval INTEGER,          -- NEW in v3: Repeat every N periods
  recurrenceEndDate TEXT,              -- NEW in v3: When to stop repeating
  recurrenceGroupId TEXT,              -- NEW in v3: Group ID for series
  createdAt TEXT NOT NULL,             -- ISO 8601 timestamp
  updatedAt TEXT                       -- ISO 8601 timestamp
);

CREATE TABLE templates (
  id TEXT PRIMARY KEY,                 -- NEW in v3: Template UUID
  name TEXT NOT NULL,                  -- Template name
  title TEXT NOT NULL,                 -- Default meeting title
  durationMinutes INTEGER NOT NULL,    -- Default duration
  description TEXT,                    -- Default description
  participants TEXT,                   -- Default participants
  category TEXT NOT NULL,              -- Default category
  reminderEnabled INTEGER NOT NULL,    -- Default reminder setting
  reminderMinutesBefore TEXT NOT NULL, -- Default reminder times
  meetingLink TEXT,                    -- Default meeting link
  createdAt TEXT NOT NULL              -- Template creation time
);
...
**Migration Strategy:**
- Database auto-upgrades from v1 → v2 → v3
- Adds new columns with NULL/default values
- Preserves all existing meeting data
- Creates templates table on upgrade
...
- **FEATURES_ADDED.md** - Detailed documentation of new features
...
4. Check the Settings screen for help information
...
- [x] ~~Meeting templates~~ ✅ IMPLEMENTED
- [ ] Export meetings to PDF/CSV
- [x] ~~Recurring meetings support~~ ✅ IMPLEMENTED
- [x] ~~Meeting notes~~ ✅ IMPLEMENTED
